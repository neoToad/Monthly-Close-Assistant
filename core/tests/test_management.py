...
class SeedDemoDataCommandTests(TestCase):
    def test_seeds_transactions_bank_feed_flags_and_summary(self) -> None:
        from core.models import CloseSummary

        out = StringIO()
        call_command("seed_demo_data", "2025-01", stdout=out)
        self.assertGreater(Transaction.objects.filter(date__month=1).count(), 0)
        self.assertGreater(BankTransaction.objects.filter(date__month=1).count(), 0)
        self.assertGreater(Flag.objects.count(), 0)
        self.assertEqual(CloseSummary.objects.filter(month="2025-01").count(), 1)
        output = out.getvalue()
        self.assertIn("seeded", output.lower())
        self.assertIn("2025-01", output)

    def test_re_running_is_idempotent(self) -> None:
        from core.models import CloseSummary

        call_command("seed_demo_data", "2025-01", stdout=StringIO())
        first_txn_count = Transaction.objects.filter(
            date__month=1, qb_transaction_id__startswith="DEMO-"
        ).count()
        first_flag_count = Flag.objects.count()
        first_summary_id = CloseSummary.objects.get(month="2025-01").id

        call_command("seed_demo_data", "2025-01", stdout=StringIO())
        second_txn_count = Transaction.objects.filter(
            date__month=1, qb_transaction_id__startswith="DEMO-"
        ).count()
        self.assertEqual(second_txn_count, first_txn_count)
        self.assertEqual(Flag.objects.count(), first_flag_count)
        self.assertEqual(CloseSummary.objects.get(month="2025-01").id, first_summary_id)

    def test_preserves_non_demo_transactions(self) -> None:
        existing = _make_txn(
            qb_transaction_id="QB-real-1",
        ...
        call_command("seed_demo_data", "2025-01", stdout=StringIO())
        self.assertTrue(
            Transaction.objects.filter(id=existing.id).exists(),
            "seed_demo_data should not delete non-demo transactions.",
        )
